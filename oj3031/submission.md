...
```text
รับค่าความเร็วกับจำนวนคนก่อน จากนั้นใช้ for loop วนตามจำนวนคน เพื่อรับพิกัด x และ y ของแต่ละคน แล้วก็เอาพิกัดมาคำนวณพื้นที่ด้วยสูตรวงกลม แล้วก็นำพื้นที่ที่ได้ไปหารด้วยความเร็วเพื่อหาเวลา และใช้ math.ceil() ปัดเวลาให้เป็นจำนวนเต็ม ละก็แสดงเวลาที่คำนวณได้ออกมาทีละคน
```
...
```text
รับค่า speed และจำนวนคน
วนลูปรับค่า x, y ของแต่ละคน
คำนวณพื้นที่จาก x^2 + y^2
เอาพื้นที่หาร speed แล้วปัดขึ้นด้วย ceil()
แสดงเวลาของแต่ละคนออกมา
```
...
```text
ทดสอบหลายคนและมีพิกัดหลายรูปแบบ
```
...
```text
ทดสอบพิกัด 0,0 ซึ่งทำให้พื้นที่เป็น 0
```
...
```text
ผลการคำนวณพื้นที่หารด้วยความเร็วได้ค่าทศนิยม จึงใช้ทดสอบว่า math.ceil() ปัดขึ้นได้ถูกต้อง
```
