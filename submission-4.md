...
```text
ตอนแรกสร้าง List p ว่าง ๆ แล้วใช้ for วน 5 รอบ เพื่อรับข้อความทีละบรรทัดมาเก็บไว้ใน p
ตรง rstrip() ใช้ตัดช่องว่างที่อยู่ด้านขวาสุดของข้อความออก
ละก็สร้าง max_len = 0 เอาไว้เก็บความยาวของข้อความที่ยาวที่สุด
แล้วใช้ for วนดูข้อความแต่ละบรรทัด ถ้าความยาวของข้อความนั้นมากกว่า max_len ก็จะเอาความยาวนั้นมาเก็บไว้ใน max_len
พอรู้แล้วว่าข้อความที่ยาวที่สุดยาวเท่าไหร่ ก็เอาค่านั้นไปกำหนดความกว้างของกรอบ โดยใช้ * จำนวน max_len + 4
ส่วนตรงกลางใช้ ljust(max_len) เพื่อเติมช่องว่างด้านขวาให้ข้อความทุกบรรทัดมีความยาวเท่ากัน แล้วใส่ * ไว้ด้านหน้าและด้านหลัง
สุดท้ายพิมพ์ * ด้านล่างอีกครั้ง เพื่อปิดกรอบ
```
...
```text
เช็กกรณีที่ข้อความทุกบรรทัดมีความยาวเท่ากัน โปรแกรมต้องเติมช่องว่างให้ถูกต้อง
```
...
```text
เช็กว่าโปรแกรมสามารถหา ข้อความที่ยาวที่สุด แล้วเอาความยาวนั้นไปกำหนดขนาดกรอบได้ไหม
```
...
```text
เช็กการใช้ rstrip() ว่าสามารถตัด ช่องว่างด้านขวาที่เกินมา ก่อนนำข้อความไปสร้างกรอบได้ถูกต้องไหม
```
